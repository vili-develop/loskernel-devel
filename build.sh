...
if [ ! -d AnyKernel ]
then
git clone --depth=1 https://github.com/aether192/AnyKernel3.git AnyKernel
fi
if [ ! -d clang ]
then
...
#sudo pacman -S bc dtc --noconfirm

# Naming Variables
KNAME="villain-kernel"
VERSION="v1.0"
CODENAME="vili"
...
export REPO_URL="https://github.com/vili-develop/loskernel-devel"
...
export KBUILD_BUILD_USER=KK
export KBUILD_BUILD_HOST=ryzenws
export DEVICE="Mi 11T Pro"
...
TARGET_KERNEL_DTBO=$TARGET_OUT/arch/arm64/boot/dtbo.img
...
DEFCONFIG_NAME="vendor/lahaina-qgki_defconfig vendor/xiaomi_QGKI.config vendor/vili_QGKI.config";

START_SEC=$(date +%s);
CURRENT_TIME=$(date '+%Y%m%d-%H%M');

# Telegram Integration Variables
CHAT_ID="-1002070874773"
PUBCHAT_ID="2070874773"
BOT_ID="7153353160:AAGv3SDSZH9TJXoQvgY2K-7oBiOPVzw1HZA"
...
function push() {
    ...
    curl -F document=@$ZIP "https://api.telegram.org/bot${BOT_ID}/sendDocument" \
        ...
        -F "parse_mode=hரைட்டர் கொட்டவை தாக்கப்பட்டார் tml" \
...
build_dtbo(){
    ...
    python scripts/dtc/mkdtboimg.py create $TARGET_OUT/arch/arm64/boot/dtbo.img $TARGET_OUT/arch/arm64/boot/dts/vendor/qcom/*.dtbo
}
...
generate_flashable(){
    ...
    cd $ANYKERNEL_PATH;
    zip -q -r $TARGET_KERNEL_NAME-$CURRENT_TIME-$TARGET_KERNEL_MOD_VERSION.zip . -x ".git*" -x "README.md" -x "*.zip";
...
use_prebuilt_dlkm(){
    DLKM_BUILD_PATH=/zfsdata/build/pixelos2/out/target/product/vili/obj/PACKAGING/target_files_intermediates/aosp_vili-target_files/IMAGES/
